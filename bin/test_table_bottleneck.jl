...
for mtx in [
    ...
    for (method, key, rng) in [
        ...
        #(PermutingPartitioner(CuthillMcKeePermuter(), SymmetricPartitioner(LazyBisectCostBottleneckSplitter(sym_model, eps))), "rcm_order__split_sym__symmetric__lazy__approx", false),
        (SymmetricPartitioner(LazyBisectCostBottleneckSplitter(sym_model, eps)), "split_sym__symmetric__lazy__approx", false),
