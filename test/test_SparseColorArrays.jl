...
ref_rownetcount(A, Φ_domain, j, j′, k) = length(intersect(union(A[:, j:(j′ - 1)].rowval), Φ_domain.prm[Φ_domain.spl[k]:Φ_domain.spl[k + 1] - 1]))
        ...
        for m = test_dims, K = 1:8
            for n = test_dims
                ...
                if nnz(A) > 0
                    ...
                    rownet = partwisecost!(hint..., args..., rownetcount(hint..., Ap))
                    colnet = partwisecost!(hint..., args..., WidthCost())
                    @testset "hint=$hint, m = $m, n = $n, K = $K" begin
                        for (j, j′, k) in test_points(10, 1:n + 1, 1:n + 1, 1:K)
                            (j, j′) = minmax(j, j′)
                            k = rand(1:K)

                            @test rownet[j, j′, k] == ref_rownetcount(A, Φ_domain, j, j′, k)
                            ...
                            for _ = 1:10
                                ...
                                @test ChainPartitioners.Step(rownet)(_j, _j′, _k) == ref_rownetcount(A, Φ_domain, j, j′, k)
